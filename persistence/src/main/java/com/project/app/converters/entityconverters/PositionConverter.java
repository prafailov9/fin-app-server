...
public class PositionConverter implements EntityConverter<Position, PositionDto> {
    ...
    private final InstrumentConverterFactory matcher;
    private InstrumentConverter<Instrument, InstrumentDto> instrumentConverter;

    public PositionConverter() {
        matcher = new InstrumentConverterFactory();
    }

    @Override
    public Position convertToEntity(PositionDto dto) {
        instrumentConverter = matcher.getConverter(dto.getInstrument().getIntrumentType());
    ...
    public PositionDto convertToDto(Position entity) {
        ...
        instrumentConverter = matcher.getConverter(entity.getInstrument().getType());
